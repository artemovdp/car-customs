...
PARTS = ["Бампер передний", "Фара", "Крыло переднее", "Капот", "Решётка радиатора",
         ...
         "Бампер задний", "Фонарь задний", "Дверь багажника", "Подвеска задняя",
         "Дверь", "Крыло заднее", "Порог", "Зеркало", "Подушки и блок SRS"]
...
- Оранжевый штампованный диск вместо штатного — это подменное колесо аукциона.
  Значит родное колесо или подвеска на этом углу повреждены: добавь
  "Диск и шина" и подвеску нужной стороны в damaged.
- Деталь, которой на месте нет совсем (пустой проём фары, отсутствующее крыло),
  — это damaged, а не отсутствие повреждения.
- Просевший или вывернутый угол, неравные зазоры капота и дверей — повод
  поставить structure = visible_damaged, даже если лонжеронов не видно."""
